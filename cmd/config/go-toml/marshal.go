	...
	case reflect.Struct:
		...
		for i := 0; i < mtype.NumField(); i++ {
			...
			if opts.include {
				...
				if underKey, has := upperToUnderline(baseKey); has {
					keysToTry = []string{baseKey, strings.ToLower(baseKey), strings.ToTitle(baseKey), underKey, strings.ToLower(underKey), strings.ToTitle(underKey)}
				} else {
...
func upperToUnderline(s string) (string, bool) {
	hasUnderline := false
	...
	return buffer.String(), hasUnderline
}
