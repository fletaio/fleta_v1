	...
	for {
		...
		if err != nil {
			if len(line) > 0 {
				...
				return nil // ignore crashed commands before txend
			}
			if err == io.EOF {
				break
			}
		...
		if (parts[0][0] == 't' || parts[0][1] == 'T') &&
			strings.ToLower(parts[0]) == "txend" {
			for _, v := range commiteds {
				if (v[0][0] == 's' || v[0][1] == 'S') &&
					(v[0][1] == 'e' || v[0][1] == 'E') &&
					(v[0][2] == 't' || v[0][2] == 'T') {
					// SET
					if len(v) < 3 || len(v) == 4 || len(v) > 5 {
						return ErrInvalid
					}
					if len(v) == 5 {
						if strings.ToLower(v[3]) != "ex" {
							return ErrInvalid
						}
						ex, err := strconv.ParseInt(v[4], 10, 64)
						if err != nil {
							return err
						}
						now := time.Now()
						...
						db.insertIntoDatabase(&dbItem{key: v[1], val: v[2]})
					}
				} else if (v[0][0] == 'd' || v[0][1] == 'D') &&
					(v[0][1] == 'e' || v[0][1] == 'E') &&
					(v[0][2] == 'l' || v[0][2] == 'L') {
					// DEL
					if len(v) != 2 {
					...
					db.deleteFromDatabase(&dbItem{key: v[1]})
				} else if (v[0][0] == 'f' || v[0][1] == 'F') &&
					strings.ToLower(v[0]) == "flushdb" {
					db.keys = btree.New(btreeDegrees, nil)
					db.exps = btree.New(btreeDegrees, &exctx{db})
					db.idxs = make(map[string]*index)
				} else {
